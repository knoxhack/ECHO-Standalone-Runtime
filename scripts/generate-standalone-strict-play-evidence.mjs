...
const INPUTS = {
  ...
  creativeInventory: 'reports/echo/standalone/client-creative-inventory-smoke.json',
...
export async function generateStandaloneStrictPlayEvidence({ root = DEFAULT_ROOT } = {}) {
  ...
  const outputs = {
    ...
    [OUTPUTS.uiSurfaces]: aggregateReport({
      ...
      requiredFor: ['ui'],
      inputs,
      include: ['uiParity', 'screenCatalog', 'creativeInventory', 'heldItemOverlay'],
    ...
    [OUTPUTS.voxelContent]: aggregateReport({
      ...
      inputs,
      include: ['modsRuntimeContent', 'creativeInventory', 'worldInteraction', 'heldItemOverlay'],
      partialIfAnyInputMissing: true,
      blockers: [],
    }),
    [OUTPUTS.blockActions]: aggregateReport({
      ...
      inputs,
      include: ['creativeInventory', 'worldInteraction'],
      partialIfAnyInputMissing: true,
      blockers: [],
    }),
