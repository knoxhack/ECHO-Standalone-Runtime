...
public final class EchoPackCoreStandaloneAdapter {
    public static final String MODULE_ID = "echopackcore";
    public static final String ADAPTERCORE_CONTRACT_ID = "echopackcore:packs/ashfall_load_plan";
    public static final String REFERENCE_PACK_ID = "ashfall";
    public static final String ASHFALL_ADDON_MODULE = "echoashfallprotocol";
    public static final String REFERENCE_ROOT_MODULE = ASHFALL_ADDON_MODULE;

    public Map<String, Object> activate() {
        Map<String, Object> loadPlan = executeLoadPlan("ashfall");
        boolean loadPlanPassed = referencePlanPassed(loadPlan);
        Map<String, Object> report = new LinkedHashMap<>();
        report.put("activated", true);
        report.put("activationStage", "packcore_standalone_load_plan_active");
        report.put("adapterCoreUsed", true);
        report.put("standaloneRuntimeCodeExecuted", true);
        report.put("moduleId", MODULE_ID);
        report.put("registeredFeatureContracts", List.of("pack.profile", "pack.repair", ADAPTERCORE_CONTRACT_ID));
        report.put("packLoadPlan", loadPlan);
        report.put("packLoadPlanExecuted", loadPlanPassed);
        report.put("packLoadPlanContract", ADAPTERCORE_CONTRACT_ID);
        report.put("serviceCodeExecuted", loadPlanPassed);
        report.put("summary", "PackCore standalone adapter executed the AdapterCore Ashfall PackOS load-plan service.");
        return Map.copyOf(report);
    }

    public Map<String, Object> executeLoadPlan(String packId) {
    ...
    public boolean referencePlanPassed(Map<String, Object> plan) {
        return Boolean.TRUE.equals(plan.get("packLoadPlanExecuted"))
                ...
                && REFERENCE_PACK_ID.equals(plan.get("packId"))
                && String.valueOf(plan.get("profile")).contains("rootModule=" + REFERENCE_ROOT_MODULE)
    ...
    private static List<Map<String, Object>> loadPlan() {
        return List.of(
                ...
                step("activate_root_gameplay_module", "ACTIVATE_ROOT_MODULE", REFERENCE_ROOT_MODULE, false),
    ...
    private static String normalizePackId(String packId) {
        String normalized = packId == null || packId.isBlank() ? REFERENCE_PACK_ID : packId.trim();
        return REFERENCE_PACK_ID.equals(normalized) || "echo-native-m17".equals(normalized) ? REFERENCE_PACK_ID : normalized;
    }
}
